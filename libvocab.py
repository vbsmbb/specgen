...
#       + In generate function, changed self.template to self.temploc
#       + Casted the second argument in the tpl.replace function to strings in the generate function
# Usage:
#
...
class VocabReport(object):
    ...
    def generate(self,htmlgroups):
        ...
        tpl = tpl.replace("%ns%",str(self.vocab._uri.encode("utf-8")))
        tpl = tpl.replace("%specurl%",str(self.specurl.encode("utf-8")))
        tpl = tpl.replace("%name%",str(self.name.encode("utf-8")))
        
        tpl = tpl.replace("%date%",str(datestring.encode("utf-8")))
        tpl = tpl.replace("%rdfadate%",str(rdfadatestring.encode("utf-8")))
        tpl = tpl.replace("%version%",str(version.encode("utf-8")))
          
          
        tpl = tpl.replace("%concepttypes%",str(self.concepttypes.encode("utf-8")))
        tpl = tpl.replace("%concepttypes2%",str(self.concepttypes2.encode("utf-8")))
        tpl = tpl.replace("%concepttypes3%",str(self.concepttypes3.encode("utf-8")))
          
        tpl = tpl.replace("%azlist%",str(azlist.encode("utf-8")))
        tpl = tpl.replace("%termlist%",str(termlist.encode("utf-8")))
        
        if (htmlgroups != None):
            tpl = tpl.replace("%groups%",str(htmlgroups.encode("utf-8")))
        else:
